...
val AccentGreen       = Color(0xFF5CB85C)
            ...
            modifier = Modifier
                ...
                .background(
                    Brush.linearGradient(
                        colors = listOf(PrimaryBlue, Color(0xFF6AB0F5))
                    )
                ),
        ...
        Column(modifier = Modifier.padding(16.dp)) {
            ...
            Row(verticalAlignment = Alignment.CenterVertically) {
                ...
                Column {
                    ...
                    Text(
                        text = "Distância 7,8 km",
                        fontSize = 13.sp,
                        color = TextLight
                    )
                    Text(
                        text = "Calorias 412 kcal",
                        fontSize = 13.sp,
                        color = TextLight
                    )
                }
...
fun EmotionalStateCard() {
    var selectedEmotion by remember { mutableStateOf(2) } // Neutro por defeito
...
@Composable
fun WaterIntakeCard() {
    val currentLiters = 1.6f
    val goalLiters    = 2.5f
    val progress      = currentLiters / goalLiters
    val cups          = 6
    val totalCups     = 10
...
// ─────────────────────────────────────────────
//  GARRAFA DE ÁGUA DESENHADA COM CANVAS
// ─────────────────────────────────────────────
@Composable
fun WaterBottle(progress: Float, modifier: Modifier = Modifier) {
    ...
    Canvas(modifier = modifier) {
        val w = size.width
        val h = size.height
        val bottleColor  = Color(0xFFE3EEFF)
        val waterColor   = PrimaryBlue.copy(alpha = 0.8f)
        val outlineColor = PrimaryBlue

        // Contorno da garrafa (simplificado)
        val bottleTop    = h * 0.2f
        val bottleLeft   = w * 0.25f
        val bottleRight  = w * 0.75f
        val bottleBottom = h * 0.95f
        val neckTop      = h * 0.05f
        ...
        val path = Path().apply {
            ...
            lineTo(neckRight, bottleTop)
            lineTo(bottleRight, bottleTop + h * 0.08f)
            lineTo(bottleRight, bottleBottom)
            lineTo(bottleLeft, bottleBottom)
            lineTo(bottleLeft, bottleTop + h * 0.08f)
            lineTo(neckLeft, bottleTop)
            close()
        }

        // Fundo da garrafa
        drawPath(path, bottleColor)

        // Água (clip para a garrafa)
        val waterLevel = bottleBottom - (bottleBottom - bottleTop) * animatedProgress
        val waterPath  = Path().apply {
            moveTo(bottleLeft, waterLevel)
            lineTo(bottleRight, waterLevel)
            lineTo(bottleRight, bottleBottom)
            lineTo(bottleLeft, bottleBottom)
            close()
        }
